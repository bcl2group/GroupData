  ...
  public void buildEvaluator (Instances data)
    ...
    for (int att_counter = 0; att_counter < data.numAttributes(); att_counter++){
        if (att_counter != classIndex){
            ...
            m_ROCs[att_counter] = ev.weightedAreaUnderROC();//* 0.25 + (0.5*ev.weightedFMeasure()) + (0.25 * ev.fMeasure(0) * ev.fMeasure(1));
            if (att_counter % 10 == 0){
                System.out.println("" + att_counter + " : " + m_ROCs[att_counter]);
            }
