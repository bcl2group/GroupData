...
double Network::getConditionalProbability(vector<bool> vb, int child)
{
	...
	for (unsigned x = 0; x < allProbabilities[child].size(); x++) {
		if (allProbabilities[child][x].first==vb) {
			cout << allProbabilities[child][x].second << endl;
	...
	for (int n = 0; n < numOfNodes; n++) {
		...
		for (int q = 0; q < power; q++) {
			...
			for (int w = 0; w < size; w++) {
				bool b = (currentInstance[w] == 0 ? true : false);
				vBools.push_back(b);
				cout << "vBools now " << b << endl;
			}
...
/**
void Network::createConditionalProbabilityTables()
{
	...
	for (int n = 0; n < numOfNodes; n++) {
		...
		vector< pair<vector<bool>, double> > APn = allProbabilities[n];

		for (unsigned q = 0; q < APn.size(); q++) {
			
			string resultT = "true        ";   
			string resultF = "false       ";

			for (unsigned w = 0; w < APn[q].first.size(); w++) {
				string boolean = APn[q].first[w] ? "true" : "false";
				resultT += boolean+ "        ";
				resultF += boolean+ "        ";
			}
