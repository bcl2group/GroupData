...
# file = 'result.write'
  ...
  call_process = "nohup cat /scratch/vinnie/GEOPipelineFiles_ubuntu/binarize_GEOpipeline.R | R --vanilla --args " + action + " " + split + " " + id + " " + threshold + " " + disease + " " + disease_ids + " | less > " + file + " &"
...
#  file = directory + "result.output"
  #file = directory + "test10.output"
  file = directory + "test12_test.output"
...
#  print "disease add ---> " + str(disease_add)
  ...
  disease_directory = make_folder(directory, disease+"_binarize")
...
#  upper_file = open("/scratch/vinnie/GEOPipelineFiles_ubuntu/mesh_ontology/upper_nodes/adenocarcinoma.txt")
  ...
  #disease_add = ["Adenocarcinoma"]
  add_disease_ids = useful_id_dict[disease_add[0]]

  #for sub_disease in disease_list: 
...
#    run_bayes(bayes_types, ["multi"], disease_id_list, thresholds, "5", disease=sub_disease, switch=True, extra=disease_add[0], extra_ids=add_disease_ids) 
    ###############################

    ##### training with just this disease #####
    #run_bayes(bayes_types, ["multi", "single"], disease_id_list, thresholds, "5", disease=sub_disease, switch=True, extra="None", extra_ids=add_disease_ids) 
    #run_bayes(bayes_types, ["multi"], disease_id_list[:1], ["20"], "5", disease=sub_disease, switch=True, extra="None", extra_ids=add_disease_ids) 
    #run_bayes(bayes_types, ["single"], disease_id_list[:1], ["50"], "5", disease=sub_disease, switch=True, extra="None", extra_ids=add_disease_ids) 
    run_bayes(bayes_types, ["single"], disease_id_list[:1], ["50"], "5", disease=sub_disease, switch=True, extra="None", extra_ids="None") 
    #run_bayes(bayes_types, ["single"], disease_id_list, thresholds, "5", disease=sub_disease, switch=True, extra="None", extra_ids=add_disease_ids) 
    ###########################################
...
########################################################################
   ...
   upper_file = open("/scratch/vinnie/GEOPipelineFiles_ubuntu/mesh_ontology/upper_nodes/dermatitis.txt")
#   upper_file = open("/scratch/vinnie/GEOPipelineFiles_ubuntu/mesh_ontology/upper_nodes/aneurysm.txt")
   disease_list = []
   for line in upper_file:
     disease_name = line[:-1]
     converted_name = convert_dict[disease_name]
...
#     run_test(disease_id_list, ["train"], ["multi"], ["20", "30", "50", "80", "90"], disease, add_disease_ids, disease_add=disease_add[0])
     run_test(disease_id_list[:1], ["train"], ["single"], ["90"], disease, disease_ids)
#     run_test(disease_id_list, actions, ["single"], thresholds, disease, disease_ids)
