...
import control_attribute_selector_mm as control
import parse_results_mm as parser
...
#		test_file_name = "HS_" + disease + "_" + experiment + "_" + split_type + "_" + str(percentage) + "_test.arff"
		base_directory = "/scratch/vinnie/GEOPipelineFiles_ubuntu/results_MM/" + disease + "_binarize/" + experiment_to_split + "/"
		train_directory = base_directory + "train/" + split_type + "/" + str(percentage) + "/"
		print "train directory ---> " + str(train_directory)
		test_directory = base_directory + "test/" + split_type + "/" + str(percentage) + "/"
		print "test directory ---> " + str(test_directory)
		train_file_name = "MM_" + disease + "_" + experiment_to_split + "_" + split_type + "_" + str(percentage) + "_train.arff"
		test_file_name = "MM_" + disease + "_" + experiment_to_split + "_" + split_type + "_" + str(percentage) + "_test.arff"
...
#	disease_id_list = ['GDS1580']
	...
	for experiment in disease_id_list:
		...
		multi_directory = "/scratch/vinnie/GEOPipelineFiles_ubuntu/results_MM/" + disease + "_binarize/" + disease_id_list[0] + "/train/multi/100/"
		multi_file_name = disease_id_list[0] + "_" + disease + "_training_multi_split_singlenet_threshold_100_10percent.txt"

		single_directory = "/scratch/vinnie/GEOPipelineFiles_ubuntu/results_MM/" + disease + "_binarize/" + experiment + "/train/single/100/"
		...
		for percentage in percentages:
			num_ints = int(float(percentage)/float(100) * length)
			multi_rand_ints = multi_rand_int_list[:num_ints]
			single_rand_ints = single_rand_int_list[:num_ints]
		
			print "percentage ---> " + str(percentage)
			print "num ints ---> " + str(num_ints)
			print "multi ints ---> " + str(multi_rand_ints)
			print "single  ints ---> " + str(single_rand_ints)
...
def get_disease_lines(disease_id_list):
	for experiment in disease_id_list:
		print "experiment ---> " + str(experiment)
		if split_type == 'multi' or split_type == 'multi_single':
			directory = "/scratch/vinnie/GEOPipelineFiles_ubuntu/results_MM/" + disease + "_binarize/" + disease_id_list[0] + "/train/" + split_type + "/100/"
			file_name = disease_id_list[0] + "_" + disease + "_training_" + split_type + "_split_singlenet_threshold_100_10percent.txt"
		else:
			directory = "/scratch/vinnie/GEOPipelineFiles_ubuntu/results_MM/" + disease + "_binarize/" + experiment + "/train/" + split_type + "/100/"
...
#control.run_bayes_net(disease, 'multi_single',multinet)	
