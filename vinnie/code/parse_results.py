...
#  writer = csv.writer(open('breast_neoplasm_results_threshold_GDS2745GDS1326.csv', 'a'))
  if multinet == True:
    writer = csv.writer(open(disease + '_results_threshold_test_multinet_50.csv', 'a'))
  else:
    writer = csv.writer(open(disease + '_results_threshold_test_50.csv', 'a'))
  ...
  #thresholds = ["80"]
  directory = '/scratch/vinnie/multinet/multinet/attributes_testing/' + disease + '_' + data_type + '/' 
  directory_multi = '/scratch/vinnie/multinet/multinet/attributes_testing/' + disease + '_multi/'
...
#		for threshold in ["30"]:
		for split in range(100,800,100):
