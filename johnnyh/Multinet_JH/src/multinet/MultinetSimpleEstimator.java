...
public class MultinetSimpleEstimator extends SimpleEstimator {
	 ...
	 * returned, since the MultiNetSimpleEstimator is associated with a single
	 * class label.
	 * 
	 * @param bayesNet
	...
	protected double logDist(BayesNet bayesNet, Instance instance) {
		...
		for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
			...
			ParentSet s = bayesNet.getParentSet(iAttribute);

			for (int iParent = 0; iParent < s.getNrOfParents(); iParent++) {
				int nParent = s.getParent(iParent);
