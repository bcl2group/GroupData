...
print(paste("Bipolar_Disorder,", percentTopGenes))
name1 = paste("Bipolar_Disorder_Singlenet, sep=")
filenameSinglenet = paste(name1, "percent.txt", sep = "")
name2 = paste("Bipolar_Disorder", sep="")
...
for (gdsName in usefulIDs) {
	...
	if (useTotalCommonGenes) numTopGenes <- sum(F)
	...
	numTopGenes <- sum(F)
	ff=exprs(eset)
	s = apply(ff,1,sd) # s= apply(gdsTable,1,sd)
	F = s>quantile(s,0.90)
	tabd = log(ff[F,])
	
	#s = apply(DATA,1,sd)
	#F = s>quantile(s,0.80)
	#DATA_FILTERED = log(DATA[F,])

	# hayi <- eset[[1]] # gives out Genenames
	geneIndexes <- (row.names(tabd))
	#geneIndexes <- as.integer(row.names(gdsTable))
...
write("% Title: Bipolar Disorder", file=filenameSinglenet)
write("% Creator: Swetha", file=filenameSinglenet, append=TRUE)
...
write("% Title: Bipolar Disorder", file=filename)
write("% Creator: Swetha Sampath", file=filename, append=TRUE)
